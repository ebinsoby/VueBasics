<template>
  <div class="questions-ctr">
    ...
    <transition-group name="fade">
      ...
      <div class="answers">
        <div class="answer"  v-for="answer in question.answers" :key="answer.text" @click.prevent="answered(answer.is_correct)">{{  answer.text }}</div>
      </div>
    </div>
    </transition-group>
...
export default {
    ...
    emits: ["answered-event"],
    methods:{
        answered(isAnswerCorrect){
            this.$emit("answered-event",isAnswerCorrect)   
        } 
    }
}; 
</script>
